...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-100">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="relative bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="relative px-8 py-8">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-white">{{ __('kb.edit_category') }}</h1>
                            <p class="mt-1 text-purple-100">Perbarui informasi kategori: {{ $category->name }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Form -->
        <form action="{{ route('kb.categories.update', $category) }}" method="POST" class="bg-white shadow-lg rounded-2xl overflow-hidden">
            @csrf
            @method('PUT')
