    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div class="relative px-8 py-6">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-green-100">{{ __('maintenance.schedules.update_schedule') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <form action="{{ route('maintenance.schedules.update', $schedule) }}" method="POST" class="bg-white shadow-sm rounded-2xl overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="frequency_type" required class="w-full border-2 border-gray-200 rounded-xl px-4 py-3">
                                <option value="daily" {{ old('frequency_type', $schedule->frequency_type) == 'daily' ? 'selected' : '' }}>{{ __('enums.maintenance_frequency.daily') }}</option>
                                <option value="weekly" {{ old('frequency_type', $schedule->frequency_type) == 'weekly' ? 'selected' : '' }}>{{ __('enums.maintenance_frequency.weekly') }}</option>
                                <option value="monthly" {{ old('frequency_type', $schedule->frequency_type) == 'monthly' ? 'selected' : '' }}>{{ __('enums.maintenance_frequency.monthly') }}</option>
                                <option value="yearly" {{ old('frequency_type', $schedule->frequency_type) == 'yearly' ? 'selected' : '' }}>{{ __('enums.maintenance_frequency.annual') }}</option>
                                <option value="custom" {{ old('frequency_type', $schedule->frequency_type) == 'custom' ? 'selected' : '' }}>{{ __('maintenance.schedules.custom_days') }}</option>
                            </select>
                        </div>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('maintenance.schedules.frequency') }}</label>
                ...
                <div>
                    <h3 class="text-lg font-bold text-gray-900 mb-4">{{ __('maintenance.schedules.assignment_cost') }}</h3>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
