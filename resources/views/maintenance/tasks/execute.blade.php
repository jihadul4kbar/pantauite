            ...
            <div class="relative bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="relative px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-white">{{ $task->task_number }} - {{ __('') }}</h1>
        ...
        <form action="{{ route('maintenance.tasks.save-execution', $task) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                            ...
                            <div class="p-4 border border-gray-200 rounded-xl {{ $item->is_required ? 'bg-red-50 border-red-200' : 'bg-white' }}">
                                <div class="flex items-start justify-between mb-3">
                                    <div class="flex-1">
                                        ...
                                        @if($item->requires_photo) <span class="inline-flex items-center px-2 py-0.5 bg-blue-100 text-blue-800 text-xs rounded mt-1">{{ __('') }}</span> @endif
                                    </div>
                                    @if($item->is_required) <span class="px-2 py-1 bg-red-100 text-red-800 text-xs font-bold rounded">{{ __('') }}</span> @endif
                                </div>
                                <div class="flex items-center space-x-4">
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        ...
                                        <span class="text-sm font-medium text-green-700">{{ __('') }}</span>
                                    </label>
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        <input type="radio" name="checklist[{{ $item->item_name }}][status]" value="fail" class="text-red-600 focus:ring-red-500">
                                        <span class="text-sm font-medium text-red-700">{{ __('') }}</span>
                                    </label>
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                ...
                                <input type="text" name="checklist[{{ $item->item_name }}][notes]" placeholder="{{ __('') }}" class="mt-3 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-500">
                            </div>
                            @endforeach
                        </div>
                    </div>
                        ...
                        <div class="p-6" id="partsContainer">
                            <div class="space-y-4 parts-list">
                                <div class="grid grid-cols-12 gap-3 items-end border-b border-gray-200 pb-2">
                                    <div class="col-span-4"><label class="text-xs font-semibold text-gray-500">{{ __('') }}</label><input type="text" name="requirements[0][part_name]" placeholder="{{ __('') }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"></div>
                                    <div class="col-span-2"><label class="text-xs font-semibold text-gray-500">{{ __('') }}</label><input type="number" name="requirements[0][quantity]" value="1" min="0" step="0.01" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm qty-input"></div>
                                    <div class="col-span-2"><label class="text-xs font-semibold text-gray-500">{{ __('') }}</label><input type="number" name="requirements[0][unit_cost]" placeholder="0" min="0" step="0.01" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm cost-input"></div>
                                    <div class="col-span-2"><label class="text-xs font-semibold text-gray-500">{{ __('') }}</label><input type="text" readonly class="w-full border border-gray-100 rounded-lg px-3 py-2 text-sm bg-gray-50 total-input" value="0"></div>
                                    <div class="col-span-2"><button type="button" onclick="addPartRow()" class="w-full px-3 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white text-sm font-semibold rounded-lg hover:from-green-600 hover:to-emerald-700">+ {{ __('') }}</button></div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                            <h2 class="text-lg font-bold text-gray-900">{{ __('') }}</h2>
                        </div>
                        <div class="p-6">
                            <textarea name="resolution_notes" rows="4" placeholder="{{ __('') }}" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-green-500 focus:border-green-500"></textarea>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
                        <div class="p-6 space-y-4">
                            ...
                            @if($task->estimated_cost) <div><label class="text-sm font-semibold text-gray-500">{{ __('') }}</label><p class="text-sm font-bold text-green-600 mt-1">Rp {{ number_format($task->estimated_cost, 0, ',', '.') }}</p></div> @endif
                        </div>
                    </div>
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <select name="photo_type" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm">
                                            <option value="before">{{ __('') }}</option>
                                            <option value="during">{{ __('') }}</option>
                                            <option value="after">{{ __('') }}</option>
                                            <option value="evidence">{{ __('') }}</option>
                                        </select>
                                    </div>
                                    <div class="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center hover:border-green-400 transition-colors cursor-pointer" onclick="document.getElementById('photoInput').click()">
                                        ...
                                        <p class="text-sm text-gray-600">{{ __('') }}</p>
                                        <p class="text-xs text-gray-400">{{ __('') }}</p>
                                    </div>
                                    ...
                                    <button type="submit" class="w-full px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white text-sm font-semibold rounded-lg hover:from-blue-600 hover:to-blue-700">{{ __('common.upload') }} {{ __('') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
function addPartRow() {
    ...
    const newRow = `
        <div class="grid grid-cols-12 gap-3 items-end border-b border-gray-200 pb-2 pt-2">
            <div class="col-span-4"><input type="text" name="requirements[${partIndex}][part_name]" placeholder="{{ __('') }}" class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm"></div>
