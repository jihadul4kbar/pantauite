    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl shadow-xl px-8 py-6">
                ...
                <div class="mt-2 text-sm text-blue-100">{{ __('maintenance.inventory.current_stock', 'Stok Saat Ini') }}: <span class="font-bold text-white">{{ $inventory->quantity_in_stock }} {{ $inventory->unit }}</span></div>
            </div>
        </div>
        ...
        <form action="{{ route('maintenance.inventory.process-stock-in', ['inventory' => $inventory->id]) }}" method="POST" class="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                <div class="grid grid-cols-2 gap-6">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('maintenance.inventory.quantity_add', 'Jumlah Ditambahkan') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="col-span-2">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('maintenance.inventory.supplier', 'Pemasok') }}</label>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="notes" rows="3" class="w-full border-2 border-gray-200 rounded-xl px-4 py-3" placeholder="{{ __('maintenance.inventory.notes_placeholder', 'Pesanan pembelian, tanda terima, dll.') }}">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
            <div class="px-6 py-5 bg-gray-50 border-t flex justify-end space-x-3">
                ...
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold rounded-xl hover:from-blue-600 hover:to-blue-700">
                    ...
                    {{ __('maintenance.inventory.add_stock', 'Tambah Stok') }}
                </button>
            </div>
        </form>
    </div>
