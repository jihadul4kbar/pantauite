...
@section('title', __('tickets.edit_category'))
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="relative bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="relative px-8 py-8">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-white">{{ __('tickets.edit_category') }}: {{ $category->name }}</h1>
                            <p class="mt-1 text-green-100">{{ __('tickets.edit_category_subtitle') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Form -->
        <form action="{{ route('tickets.categories.update', $category) }}" method="POST" class="bg-white shadow-sm hover:shadow-lg rounded-2xl overflow-hidden transition-shadow">
            @csrf @method('PUT')
                    ...
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" rows="3" class="w-full border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors px-4 py-3 resize-none">{{ old('description', $category->description) }}</textarea>
                            </div>
                            <div>
                                ...
                                <select name="parent_id" class="w-full border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors px-4 py-3">
                                    ...
                                    <option value="{{ $parent->id }}" {{ old('parent_id', $category->parent_id) == $parent->id ? 'selected' : '' }}>{{ $parent->icon }} {{ $parent->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                            ...
                            <div class="flex items-center">
                                <label class="flex items-center p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 cursor-pointer hover:from-green-100 hover:to-emerald-100 transition-colors">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} class="rounded border-2 border-gray-300 text-green-600 focus:ring-green-500 w-5 h-5">
