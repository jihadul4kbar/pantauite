...
@section('title', __('departments.add_new'))
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="relative bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="relative px-8 py-8">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-white">{{ __('departments.add_new') }}</h1>
                            <p class="mt-1 text-green-100">{{ __('departments.create_subtitle') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Form -->
        <form action="{{ route('departments.store') }}" method="POST" class="bg-white shadow-sm hover:shadow-lg rounded-2xl overflow-hidden transition-shadow">
            @csrf
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="manager_id" class="w-full border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors px-4 py-3">
                                    ...
                                    <option value="{{ $manager->id }}" {{ old('manager_id') == $manager->id ? 'selected' : '' }}>{{ $manager->name }} ({{ $manager->role->display_name }})</option>
                                    @endforeach
                                </select>
                            </div>
