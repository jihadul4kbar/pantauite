...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-green-50 to-emerald-50">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Header -->
        <div class="mb-6">
            <a href="{{ route('departments.index') }}" class="group inline-flex items-center space-x-2 text-sm text-gray-600 hover:text-green-600 mb-4 transition-colors">
            ...
            <div class="relative bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 rounded-2xl shadow-xl overflow-hidden">
                ...
                <div class="relative px-8 py-8">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 class="text-3xl font-bold text-white">{{ __('departments.edit_department') }}: {{ $department->name }}</h1>
                            <p class="mt-1 text-green-100">{{ __('departments.edit_subtitle') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- Form -->
        <form action="{{ route('departments.update', $department) }}" method="POST" class="bg-white shadow-sm hover:shadow-lg rounded-2xl overflow-hidden transition-shadow">
            @csrf @method('PUT')
                ...
                <div class="bg-white shadow-sm rounded-2xl overflow-hidden">
                    <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white">
                        <div class="flex items-center space-x-3">
                            ...
                            <h2 class="text-lg font-bold text-gray-900">{{ __('departments.basic_info') }}</h2>
                        </div>
                    </div>
                    <div class="p-6 space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('departments.department_name') }} <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('common.code') }} <span class="text-red-500">*</span></label>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea name="description" rows="3" class="w-full border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors px-4 py-3 resize-none">{{ old('description', $department->description) }}</textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('departments.parent_department') }}</label>
                                <select name="parent_id" class="w-full border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors px-4 py-3">
                                    <option value="">{{ __('departments.none_root') }}</option>
                            ...
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">{{ __('departments.department_manager') }}</label>
                                <select name="manager_id" class="w-full border-2 border-gray-200 rounded-xl shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors px-4 py-3">
                                    <option value="">{{ __('departments.no_manager') }}</option>
                                    @foreach($managers as $manager)
                                    <option value="{{ $manager->id }}" {{ old('manager_id', $department->manager_id) == $manager->id ? 'selected' : '' }}>{{ $manager->name }} ({{ $manager->role->display_name }})</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="flex items-center">
                                <label class="flex items-center p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-200 cursor-pointer hover:from-green-100 hover:to-emerald-100 transition-colors">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $department->is_active) ? 'checked' : '' }} class="rounded border-2 border-gray-300 text-green-600 focus:ring-green-500 w-5 h-5">
            ...
            <div class="px-6 py-5 bg-gradient-to-r from-gray-50 to-white border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between space-y-3 sm:space-y-0">
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('departments.index') }}" class="group px-6 py-3 bg-gray-100 text-gray-700 rounded-xl hover:bg-gray-200 font-semibold transition-colors shadow-sm">
