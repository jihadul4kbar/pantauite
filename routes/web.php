...
Route::prefix('repair-requests')->name('repair-requests.')->group(function () {
    ...
    Route::post('/store', [RepairRequestController::class, 'store'])->name('store');
    Route::get('/success/{requestNumber}', [RepairRequestController::class, 'success'])->name('success');
});
...
Route::middleware(['auth', 'password.expired'])->group(function () {
    ...
    Route::prefix('tickets/categories')->name('tickets.categories.')->group(function () {
    ...
    Route::post('tickets/{ticket}/link-kb', [TicketController::class, 'linkKbArticle'])
    ...
    // SLA pause/resume
    Route::post('tickets/{ticket}/sla/pause', [TicketController::class, 'toggleSlaPause'])
    ...
    // KB Image Upload
    Route::post('kb/upload-image', [KbArticleController::class, 'uploadImage'])->name('kb.upload-image');
    
    // KB Category Management (IT staff/manager only)
    Route::middleware('can:viewAny,App\Models\KbCategory')->prefix('kb-categories')->name('kb.categories.')->group(function () {
    ...
    Route::post('assets/{asset}/maintenance', [AssetController::class, 'logMaintenance'])
    ...
    Route::prefix('maintenance')->name('maintenance.')->group(function () {
        // Schedules
        Route::resource('schedules', MaintenanceScheduleController::class)->except(['show']);
        Route::post('schedules/generate-tasks', [MaintenanceScheduleController::class, 'generateTasks'])
            ->name('schedules.generate-tasks');

        // Tasks
        Route::resource('tasks', MaintenanceTaskController::class);
        Route::get('tasks/{task}/execute', [MaintenanceTaskController::class, 'execute'])
            ->name('tasks.execute');
        Route::post('tasks/{task}/execute', [MaintenanceTaskController::class, 'saveExecution'])
            ->name('tasks.save-execution');
        Route::post('tasks/{task}/photos', [MaintenanceTaskController::class, 'uploadPhoto'])
            ->name('tasks.upload-photo');
        Route::get('tasks/{task}/approval', [MaintenanceTaskController::class, 'requestApproval'])
            ->name('tasks.request-approval');
        Route::post('tasks/{task}/approve', [MaintenanceTaskController::class, 'approve'])
            ->name('tasks.approve');
        Route::post('tasks/{task}/reject', [MaintenanceTaskController::class, 'reject'])
            ->name('tasks.reject');

        // Inventory Parts
        Route::resource('inventory', InventoryPartController::class);
        Route::post('inventory/{inventory}/adjust', [InventoryPartController::class, 'adjustStock'])
            ->name('inventory.adjust');
        Route::get('inventory/{inventory}/stock-in', [InventoryPartController::class, 'stockIn'])
            ->name('inventory.stock-in');
        Route::post('inventory/{inventory}/stock-in', [InventoryPartController::class, 'processStockIn'])
    ...
    Route::prefix('reports')->name('reports.')->group(function () {
        ...
        Route::post('/tickets', [ReportController::class, 'generateTicketReport'])->name('tickets');
        Route::post('/assets', [ReportController::class, 'generateAssetReport'])->name('assets');
        Route::post('/kb', [ReportController::class, 'generateKbReport'])->name('kb');
    });

    // Repair Request Verification (IT Manager & Super Admin only)
    Route::prefix('admin/repair-requests')->name('repair-requests.admin.')->group(function () {
        ...
        Route::post('/{repairRequest}/approve', [RepairRequestController::class, 'approve'])->name('approve');
        Route::post('/{repairRequest}/reject', [RepairRequestController::class, 'reject'])->name('reject');
        Route::post('/{repairRequest}/convert', [RepairRequestController::class, 'convertToTicket'])->name('convert');
    });
});
